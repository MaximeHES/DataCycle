...
BRONZE_DIR = Path(r"C:\RawData\Eversys\Rinse_History")
SILVER_DIR = Path(r"C:\RawData\Eversys_Cleaned\Rinse_History")
...
def clean_rinse_file(file_path: Path) -> pd.DataFrame:
    df = pd.read_csv(
        file_path,
        sep=";",
        encoding="utf-8-sig",
    ...
    required_columns = ["machine_id", "timestamp", "rinse_type"]
    ...
    left_active = (
        ...
        if "flow_rate_left" in df.columns else pd.Series(False, index=df.index)
    )
    right_active = (
        df["flow_rate_right"].fillna(0) > 0
        if "flow_rate_right" in df.columns else pd.Series(False, index=df.index)
    )
...
def main():
    ...
    print(f"Found {len(files)} Rinse_History file(s).")
    ...
    for file_path in files:
        try:
            ...
            df_clean = clean_rinse_file(file_path)
    ...
    print("Rinse_History cleaning finished.")
