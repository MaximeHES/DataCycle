...
# Log a progress message every N candidate files while classifying delta
$ProgressEvery = 5000
...
function ConvertTo-HashtableCompat {
  ...
  if ($InputObject -is [System.Collections.IEnumerable] -and -not ($InputObject -is [string])) {
    ...
    foreach ($item in $InputObject) {
      $arr += ,(ConvertTo-HashtableCompat $item)
    }
    return $arr
  }
...
function Load-State {
  ...
  try {
    $rawJson = Get-Content -Path $StateFile -Raw -Encoding UTF8
    $rawObject = $rawJson | ConvertFrom-Json
    $raw = ConvertTo-HashtableCompat $rawObject
    if ($null -eq $raw -or -not ($raw -is [hashtable]) -or -not $raw.ContainsKey('categories')) {
...
Write-Log "ProgressEvery: $ProgressEvery candidate file(s)"
Write-Log ""
...
try {
  ...
  $globalTimer = Start-StepTimer
  $stateLoadTimer = Start-StepTimer
  $state = Load-State
  $stateLoadSeconds = Stop-StepTimer $stateLoadTimer
  Write-Log "State loaded in ${stateLoadSeconds}s"

  $cutoff = (Get-Date).AddMinutes(-$MinAgeMinutes)
  Write-Log "Cutoff time for partial-file protection: $($cutoff.ToString('o'))"
  ...
  foreach ($rule in $Rules) {
    ...
    $categoryTimer = Start-StepTimer

    # Still enumerates source matches, but avoids re-checking destination history.
    $enumTimer = Start-StepTimer
    $files = Get-ChildItem -Path $ShareRoot -File -Filter $pattern -ErrorAction Stop |
             Where-Object { $_.LastWriteTime -lt $cutoff }
    $enumSeconds = Stop-StepTimer $enumTimer

    Write-Log "Found $($files.Count) source file(s) matching pattern in ${enumSeconds}s"

    if ($files.Count -gt 0) {
      $sampleFirst = $files | Select-Object -First 1
      $sampleLast  = $files | Sort-Object LastWriteTimeUtc | Select-Object -Last 1
      Write-Log "Oldest visible source file UTC: $($sampleFirst.LastWriteTimeUtc.ToString('o')) :: $($sampleFirst.Name)"
      Write-Log "Newest visible source file UTC: $($sampleLast.LastWriteTimeUtc.ToString('o')) :: $($sampleLast.Name)"
    }

    $deltaTimer = Start-StepTimer
    $deltaFiles = New-Object System.Collections.Generic.List[object]
    $checkedCount = 0

    foreach ($f in $files) {
      $checkedCount++
      if ($ProgressEvery -gt 0 -and ($checkedCount % $ProgressEvery) -eq 0) {
        Write-Log "Progress [$cat]: checked $checkedCount / $($files.Count) source file(s)..."
      }
    ...
    $deltaSeconds = Stop-StepTimer $deltaTimer

    Write-Log "Delta file(s) to copy: $($deltaFiles.Count) (classified in ${deltaSeconds}s)"
    ...
    foreach ($item in ($deltaFiles | Sort-Object SourceUtc, @{ Expression = { $_.SourceFile.Name } })) {
      ...
      if (($copyIndex % 500) -eq 0) {
        Write-Log "Copy progress [$cat]: processed $copyIndex / $($deltaFiles.Count) delta file(s)..."
      }
    ...
    $copySeconds = Stop-StepTimer $copyTimer
    $categorySeconds = Stop-StepTimer $categoryTimer
    ...
    Write-Log "Copy phase duration: ${copySeconds}s"
    Write-Log "Total category duration: ${categorySeconds}s"
  }
  ...
  $globalSeconds = Stop-StepTimer $globalTimer
  Write-Log "Duration: $($duration.ToString())"
  Write-Log "Total runtime seconds: ${globalSeconds}s"
}
catch {
  $failure = $true
